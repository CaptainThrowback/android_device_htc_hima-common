...
if [ -f /s/build.prop ]; then
	...
	if [ ! -z "$device" ]; then
		...
		sed -i "s/ro.product.device=.*/ro.product.device="$device"/g" /default.prop ;
	fi
	if [ ! -z "$fingerprint" ]; then
		resetprop ro.build.fingerprint "$fingerprint"
		sed -i "s/ro.build.fingerprint=.*/ro.build.fingerprint="$osver"/g" /default.prop ;
	fi
	if [ ! -z "$osver" ]; then
	...
	if [ ! -z "$product" ]; then
		...
		sed -i "s/ro.build.product=.*/ro.build.product="$product"/g" /default.prop ;
	fi
	finish
else
